...
# 接続情報を保存
cat > mlbot_connection_info.txt << INFO
======================================
MLBot AWS接続情報
======================================
...
セットアップ完了後:
1. モデルファイルをアップロード:
   scp -i ~/.ssh/${KEY_NAME}.pem models/*.pth ubuntu@$PUBLIC_IP:~/mlbot/models/
   scp -i ~/.ssh/${KEY_NAME}.pem models/*.pkl ubuntu@$PUBLIC_IP:~/mlbot/models/
...
echo -e "\n${GREEN}======================================${NC}"
echo -e "${GREEN}デプロイメント準備完了！${NC}"
echo -e "${GREEN}======================================${NC}"
echo ""
cat mlbot_connection_info.txt

# セットアップスクリプトをアップロード
echo -e "\n${YELLOW}セットアップスクリプトをアップロード中...${NC}"
sleep 10  # インスタンスの初期化待ち
scp -o StrictHostKeyChecking=no -i ~/.ssh/${KEY_NAME}.pem setup_mlbot.sh ubuntu@$PUBLIC_IP:~/

echo -e "\n${GREEN}✅ 完了！次のステップ:${NC}"
echo "1. モデルファイルをアップロード"
echo "2. SSHで接続してセットアップを実行"
echo ""
echo "詳細は mlbot_connection_info.txt を参照してください"