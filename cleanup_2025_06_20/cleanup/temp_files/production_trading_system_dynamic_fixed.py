...
class ProductionTradingSystem:
    ...
    def __init__(self):
        ...
        # Initialize order router (will update its config dynamically)
        self.order_router = OrderRouter()
        
        # Get direct references to components
        self.order_executor = self.order_router.order_executor
        self.position_manager = self.order_router.position_manager
    ...
    def _update_component_configs(self, account_balance: float):
        """コンポーネントの設定を動的に更新"""
        # Create new configs
        ...
        self.order_router.risk_manager.config = risk_config
        
        # Update order executor config
        self.order_router.order_executor.config = execution_config
        
        # Update initial equity for risk manager
        self.order_router.risk_manager.peak_equity = account_balance
        self.order_router.risk_manager.current_equity = account_balance
        
        logger.info(f"Dynamic parameters updated for ${account_balance:.2f} account:")
    ...
    async def start(self):
        ...
        try:
            ...
            if self.account_monitor.current_balance:
                ...
                # Update component configs with dynamic values
                self._update_component_configs(account_balance)
                ...
                fields = {
                    ...
                    "Max Position": f"${self.order_router.risk_manager.config.max_position_size_usd:.2f}",
                    "Max Exposure": f"${self.order_router.risk_manager.config.max_total_exposure_usd:.2f}",
                    "API Status": "✅ 本番環境接続",
                ...
                account_balance = 100.0  # Assume $100
                self._update_component_configs(account_balance)
            ...
            # Position manager doesn't have start method - already initialized
            logger.info("Position manager initialized (no start method needed)")
            ...
            try:
                if self.account_monitor.current_balance:
                    ...
                    if abs(current_max_position - expected_max_position) / current_max_position > 0.1:
                        ...
                        # Update configs
                        self._update_component_configs(current_balance)
                        ...
                        discord_notifier.send_notification(
                            ...
                            fields={
                                ...
                                "Max Position Size": f"${self.order_router.risk_manager.config.max_position_size_usd:.2f}",
                                "Max Total Exposure": f"${self.order_router.risk_manager.config.max_total_exposure_usd:.2f}",
                                "Max Daily Loss": f"${self.order_router.risk_manager.config.max_daily_loss_usd:.2f}"
                            }
                        )
    ...
    async def stop(self):
        ...
        # Position manager doesn't have stop method - will be cleaned up automatically
